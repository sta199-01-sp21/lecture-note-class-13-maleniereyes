...
What is the downside to using point estimates? You are more likely to include
the correct guess if you use an interval instead. 
...
I would expect another about 5 students to be left handed. I would not be 
surprised if there were only 3, but I would be surprised if there were 40 
because the original probability was only about 10%. 
...
What does this mean? It means that in repeated samples, we are 95% confident
that our intervals contain the true value. 
...
What does this mean? We are 95% confident that true proportion of mice that died
while not on treatment is somewhere between 69% and 79.7%. 
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop_2, level = .90)
ci_mice_lived
```
...
visualise(boot_prop_2) +
  shade_ci(ci_mice_lived)
```


Give an interpretation of your result.
There is 90% confidence that the true proportion of mice that lived while on
treatment is between 55.2% and 65.1%. 
